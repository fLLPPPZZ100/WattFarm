...
/**
 * Support presentation. Prices deliberately absent: they come from the
 * catalogue, which is the only authoritative source. Hardcoding them here is
 * what let this page advertise 25 VLT for a double mount the server sells for
 * 45.
 */
var SUPPORTS = [
  { id: 'panel-mount', label: 'Panel Mount (1 Slot)', description: 'Ground mount for 1 solar panel', color: '#8B7355', img: mountSingleImg },
  { id: 'panel-mount-double', label: 'Panel Mount (2 Slots)', description: 'Ground mount for 2 solar panels', color: '#8B7355', img: mountDoubleImg },
];
...
export default function Storage() {
  var { user } = useAuth();
  var { assets, catalog, fetchMining, fetchCatalog } = useAssetsStore();
  var { placedSolar, placedMountSingle, placedMountDouble } = usePlacementStore();
  ...
  useEffect(function () {
    if (!user) return;
    fetchMining();
    // The catalogue is what prices the inventory. It changes only on purchase,
    // so it is fetched once rather than polled alongside the balance.
    fetchCatalog();
    pollingRef.current = setInterval(fetchMining, 8000);
    return function () { clearInterval(pollingRef.current); };
  }, [user, fetchMining, fetchCatalog]);
  ...
  /**
   * Storage shows what is *not* on the field, so placed items are subtracted
   * per asset type. Doubles used to subtract a literal zero while the combined
   * mount total came off the singles, so placing a double overstated storage by
   * one and understated singles by one.
   */
  ownedMap['solar'] = Math.max(0, (ownedMap['solar'] || 0) - (placedSolar || 0));
  ownedMap['panel-mount'] = Math.max(0, (ownedMap['panel-mount'] || 0) - (placedMountSingle || 0));
  ownedMap['panel-mount-double'] = Math.max(
    0,
    (ownedMap['panel-mount-double'] || 0) - (placedMountDouble || 0)
  );
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {SUPPORTS.map(function (item) {
                  ...
                  var unitPrice = priceMap[item.id] || 0;
                  return (
                      ...
                      {hasItem ? (
                        <>
                          <div className="flex items-center justify-center gap-1.5 py-2 rounded-lg border border-accent-watt/30 bg-accent-watt/5 text-accent-watt text-sm font-mono">
                            <span>🪙</span>
                            <span>{fmt(unitPrice * qty)} VLT</span>
                          </div>
