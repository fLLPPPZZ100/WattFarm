...
/** Base cost for the first grid expansion (row 5). */
const GRID_EXPANSION_BASE_COST = 50_000;

/** Geometric multiplier between successive expansion tiers. */
const GRID_EXPANSION_MULTIPLIER = 4;

/** Default rows every player starts with. */
const GRID_DEFAULT_ROWS = 4;

/** Hard cap: maximum rows a player can ever have. */
const GRID_MAX_ROWS = 8;

/** Fixed number of columns in the farm grid. */
const GRID_COLS = 14;

/**
 * Computes the VLT cost of expanding from `currentRows` to `currentRows + 1`.
 *
 * Formula: 50000 × 4^(currentRows - 4)
 *   Row 5:     50,000
 *   Row 6:    200,000
 *   Row 7:    800,000
 *   Row 8:  3,200,000
 *
 * Returns null when the player is already at max.
 *
 * @param {number} currentRows
 * @returns {Prisma.Decimal | null}
 */
function gridExpansionCost(currentRows) {
  if (currentRows >= GRID_MAX_ROWS) return null;
  const exponent = currentRows - GRID_DEFAULT_ROWS;
  const base = new Prisma.Decimal(GRID_EXPANSION_BASE_COST);
  const mult = new Prisma.Decimal(GRID_EXPANSION_MULTIPLIER);
  return base.mul(mult.pow(exponent));
}

/**
 * GET /api/assets/grid-info — current grid state and next expansion cost.
 */
router.get('/grid-info', verifyAuth, async (req, res) => {
  try {
    ...
    const currentRows = user.gridRows;
    const nextCost = gridExpansionCost(currentRows);

    return res.json({
      currentRows,
      maxRows: GRID_MAX_ROWS,
      nextExpansionCost: nextCost ? moneyToNumber(nextCost) : null,
      totalCells: GRID_COLS * currentRows,
    });
  } catch (err) {
    console.error('Grid-info error:', err);
    return res.status(500).json({ error: 'Failed to load grid info' });
  }
});

/**
 * POST /api/assets/expand-grid — purchase one extra row for the farm grid.
 *
 * Cost is geometric: 50000 × 4^(currentRows - 4). Max 8 rows.
 * Uses withUserLock for atomic balance check + debit.
 */
router.post(
  '/expand-grid',
  verifyAuthStrict,
  ...
  async (req, res) => {
    try {
      const result = await withUserLock(req.uid, async (tx, user) => {
        // withUserLock's SELECT does not include gridRows, so fetch it under
        // the same transaction (the row is already locked).
        const fullUser = await tx.user.findUnique({
          where: { id: req.uid },
          select: { gridRows: true },
        });

        const currentRows = fullUser.gridRows;

        if (currentRows >= GRID_MAX_ROWS) {
          return { maxReached: true, currentRows };
        }

        const cost = gridExpansionCost(currentRows);

        if (!canAfford(user.vltBalance, cost)) {
          return {
            insufficient: true,
            balance: money(user.vltBalance),
            cost,
        ...
        const updatedUser = await tx.user.update({
          where: { id: req.uid },
          data: {
            vltBalance: { decrement: cost },
            gridRows: { increment: 1 },
          },
        });
        ...
        return {
          success: true,
          newGridRows: updatedUser.gridRows,
      ...
      if (result.maxReached) {
        return res.status(400).json({
          error: 'Grid is already at maximum size',
      ...
      if (result.insufficient) {
        return res.status(400).json({
          error: 'Insufficient VLT balance',
          required: moneyToNumber(result.cost),
          balance: moneyToNumber(result.balance),
        });
      }
      ...
      return res.json({
        success: true,
        gridRows: result.newGridRows,
        newBalance: moneyToNumber(result.newBalance),
        nextExpansionCost: result.newGridRows < GRID_MAX_ROWS
          ? moneyToNumber(gridExpansionCost(result.newGridRows))
          : null,
        totalCells: GRID_COLS * result.newGridRows,
      });
    } catch (err) {
      if (err instanceof UserNotFoundError) {
        return res.status(404).json({ error: 'User not found' });
      }
      console.error('Expand-grid error:', err);
      return res.status(500).json({ error: 'Failed to expand grid' });
    }
  }
);
