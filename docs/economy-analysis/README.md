...
> **SUPERSEDED — kept for history only. See [`redesign-2026-07-28.md`](./redesign-2026-07-28.md).**
>
> This document, along with `income-sources.md`, `spending-sinks.md` and
> `progression-model.md`, was written on 2026-07-27 and is now factually wrong
> about the shipped code:
>
> - "P0 — exponential pricing not implemented" — it *is* implemented, in
>   `assets.js::computeExponentialTotal`.
> - "Starting grant: 0 VLT" — `auth.js` grants 50 VLT (`STARTING_VLT`).
> - "No endgame sink" — grid expansion shipped (`POST /assets/expand-grid`).
> - Minigame EV is 0.55 VLT/play, not 0.175.
> - The wind (25 VLT / 1.18x / 3 W) and hydro (60 VLT / 1.22x / 8 W) assets
>   described here do not exist in `seed.js`.
>
> Its recommendations R1–R3 were implemented, and two of them made the economy
> worse in ways this document did not predict — exponential pricing produced a
> 12,900x ROI drift, and grid expansion priced at `50,000 x 4^(rows-4)` has an
> 8-to-5,770-year payback. The successor document derives prices from a target
> payback instead, and its numbers are generated by
> `apps/api/scripts/economy-sim.mjs` rather than estimated by hand.
